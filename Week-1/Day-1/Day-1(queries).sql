...
-- 42.Select employees and number of projects their department is working on.
...
-- 43.Select the department that have no employees.
SELECT d.name
FROM Department d 
LEFT JOIN Employee e ON d.department_id = e.department_id
...
FROM Employee e
JOIN Department d ON e.department_id = d.department_id
WHERE d.department_id = (SELECT department_id FROM Employee WHERE name = 'John Doe');
...
-- 47.Select the employees whose salary is above the average salary.
...
-- 50.Select the employees who earn more than the average salary of their department.
...
-- 54.Select employees who work in a department with atleast 2 projects.
SELECT * FROM Employee e
WHERE department_id IN (SELECT department_id FROM Project GROUP BY department_id HAVING COUNT(*) >= 2);

-- 55.Select employees who were hired in the same year as 'Jane Smith'.
SELECT * FROM Employee
WHERE YEAR(hire_date) = (SELECT YEAR(hire_date) FROM Employee WHERE name = 'Jane Smith');
...
SELECT department_id, COUNT(*) AS num_employees, AVG(salary) AS avg_salary FROM Employee
...
SELECT d.name AS department_name, COUNT(p.project_id) AS num_projects FROM Department d
LEFT JOIN Project p ON d.department_id = p.department_id
GROUP BY d.name ORDER BY num_projects;

-- 64.Select the employee name with the highest salary in each department.
SELECT department_id, name, salary FROM Employee e
WHERE salary = (SELECT MAX(salary) FROM Employee WHERE department_id = e.department_id);

-- 65.Select the names and salary of employees who are older than the average age of employees in their department.
SELECT name, salary FROM Employee e
WHERE age > (SELECT AVG(age) FROM Employee WHERE department_id = e.department_id);